...
<body onload="tuijian_change()">
	<form action="Upload_file?update_id=${list.get(0).shuoshuo_id}&Tag=1"
		...
		<div class="content">
			...
			<div class="basic-content">
				...
				<div class="basic-content-cont">
					...
					<p class="content-title">内容图片
					<div class="content-picture clearfix"
						style="margin-left: 160px;padding-right:20px">
						<%
							for (int i = 0; i < list.get(0).getPictures().size(); i++) {
						%>

						<p
							style="margin-right:15px;margin-top:10px;width:100px; height:50px; background:url(${list.get(0).pictures.get(0).picture_url})no-repeat center center;background-size:cover;">
						</p>
						<%
							}
						%>
					...
					<p style="margin-top:10px;margin-left:50px">
						<input type="file" name="filepath" multiple="true" width="20px"
							height="100px" id="filepath" />
					</p>
					</p>
					<p class="content-title">内容:</p>
					<textarea type="text" class="content-input" name="shuoshuo-content"
						style=" width: 29%; height: 200px; margin-left: 12%;resize:none ">${list.get(0).content}</textarea>

...
import com.magicmoble.luzhouapp.model.My_friend_count;

public class My_friend_CountInq extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (my_id == null) {
				...
				My_friend_count count = new My_friend_count();
				count.setGuanzhu_count(GuanzhuBusiness.getFriend_id(my_id, 0).size());
				count.setFensi_count(GuanzhuBusiness.getMy_id(my_id, 0).size());
				List<My_friend_count> list = new ArrayList<My_friend_count>();
				list.add(count);

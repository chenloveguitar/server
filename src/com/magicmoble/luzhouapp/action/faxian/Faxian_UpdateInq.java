...
public class Faxian_UpdateInq extends HttpServlet {
			...
			if (map.get("id") == null) {
				...
				for (int i = 0; i < map.size(); i++) {
					if (str.split("&")[i].split("=")[0].equals("picture")) {
						FaxianBusiness.UpdateFaxian(str.split("&")[i].split("=")[0], picture, map.get("id"));
					} else {
						FaxianBusiness.UpdateFaxian(str.split("&")[i].split("=")[0], str.split("&")[i].split("=")[1],
								map.get("id"));
					}
				}
				DataObject dataObject = new DataObject();
